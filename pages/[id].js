...
export async function getServerSideProps(context) {
  // using context, can get id of the query; upon clicking on a tweet, go to dynamic route of id
  const id = context.query.id;
  // use id to get fetch tweet info from firebase
  const docRef = doc(db, "posts", id);
  //   get doc & access data inside docSnap
  const docSnap = await getDoc(docRef);
  // Must format data first before passing as a prop into commentsPage
  // data has "timestamp", which is an obj, can't pass objects in components
  ...
  //   return object after setting data
  return {
    props: {
...
export default function CommentsPage({ tweetData }) {
  ...
  return (
   ...
   "
      ...
      sticky top-0 z-50 flex space-x-1"
          >
            <Link href={"/"}>
          ...
          <div className="flex justify-between items-center p-2 border-gray-700 border-b">
            <div className="flex items-center justify-center p-1 space-x-2">
              {/* use useSelector to get current user img */}
              <img
